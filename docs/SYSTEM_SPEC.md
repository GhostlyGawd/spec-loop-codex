...
## ChatGPT Work execution in v0.7

The shared skills and Python engine can run in ChatGPT Work. The Work entry can be saved as one personal skill with a self-contained resource bundle. Capability checks select executable work, design-only work or a precise missing-access result; Codex is not a prerequisite. Project state remains separate from skill installation. Resume requires the saved source and native records, followed by fresh context and roadmap derivation. See [Work mode](../skills/spec-loop/references/work-mode.md) and [CHANGE-007](CHANGE-007.md).
