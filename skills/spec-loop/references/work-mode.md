...
The personal package has one SKILL.md entry; its lifecycle workflows use GUIDE.md resource files with adjusted local links. The shared plugin keeps its original skill entries. The bundle records source commit/tree, file hashes and payload digest in `work-bundle.json`. It excludes the plugin repository's root product records and untracked files; bundled examples remain synthetic. Compare `install_check.py --work-bundle --expected-digest` with a trusted recorded digest when checking a restored copy. Hash inventories are unsigned. For an update, prepare and validate the new bundle before replacing only this skill's old bundle through its supported save flow; preserve other personal skills.
